...
async function main() {
  ...
  const crossTenant = await executeOpenAIWorkflow({
  ...
  const tenantMismatch = await executeOpenAIWorkflow({
  ...
  const lowRisk = await executeOpenAIWorkflow({
    tenantId: INTERNAL_TENANT_ID,
  ...
  assert.strictEqual(lowRisk.executionMode, 'stubbed');
  assert.strictEqual(lowRisk.escalationCase, null);
  assert(lowRisk.risk.riskIndex <= 0.5);

  const highRisk = await executeOpenAIWorkflow({
    tenantId: INTERNAL_TENANT_ID,
  ...
  assert.strictEqual(highRisk.executionMode, 'escalated_for_human_review');
  ...
  assert(entries.every((entry) => typeof entry.executionMode === 'string'));
  assert(entries.every((entry) => typeof entry.provider === 'string'));
