...
Treat the connector as code-present until a deployed GPT action proves the connection. Use the runtime verifier with the public SentinelOS URL and a tenant-scoped key:

```bash
SENTINEL_GPT_ACTION_BASE_URL=https://<sentinel-host> \
SENTINEL_GPT_ACTION_API_KEY=<sentinel-api-key> \
SENTINEL_GPT_ACTION_TENANT_ID=nunn-internal \
npm run verify:gpt-action-connection
```

The verifier closes the evidence gap only when all of these pass in the deployed environment:

- `GET /faceplane/openai/gpt-actions/openapi.json` returns the GPT Action schema.
- The schema includes `getSentinelGPTConnectionStatus`, `getSentinelHealth`, and `getSentinelReadiness`.
- `GET /health` succeeds.
- `GET /ready` succeeds.
- `GET /faceplane/openai/gpt-actions/connection` returns `gptAction: "connected"`.

Until that runtime check passes from the same host configured in the GPT, classify the connection as `CODE PATH PRESENT / RUNTIME CONNECTION UNVERIFIED`.
