...
This refresh does not authorize push, deployment, runtime mutation, command changes, API contract renaming, Mission Control UI implementation, Microsoft Sentinel implementation, `docs/governance/PRODUCT.md` edits, publication expansion, external sharing, memory activation, cleanup, file movement, file deletion, branch settings changes, or proof claims beyond the recorded checks.
