...
(async () => {
  for (const p of ["/health","/version","/sovereignty","/runtime"]) {
  ...
  const latest = await j("/evidence/latest");
  if (!latest.r.ok || !latest.b.evidence || latest.b.evidence.state !== "verified") {
    throw new Error("evidence/latest check failed");
  }
